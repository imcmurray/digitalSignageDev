...
global slides_service, drive_service, sheets_service

def processUpdate(conf_id,ds_id):
    print('Hit digSig(processUpdate:%s|%s)'%(conf_id,ds_id))

    setup_google_services()
    # Query the configuration file id and pickup some important values for processing
    template_id = getTemplateId(conf_id,ds_id)
    template_file_id = getTemplateFileId(conf_id,template_id)
    print('Template ID found was [%s(%s)]'%(template_id, template_file_id))
    #Find out the destination folder and Quick Nav file id
    destinations = getDestinationValues(conf_id,ds_id)
    print('destinations:\n%s'%destinations)
    courtrooms = getCourtrooms(conf_id, ds_id)
    print('courtrooms:\n%s'%courtrooms)
    # Now we know which courtrooms are to displayed, find out which judges we need to grab data for
    judges,courtroomsAssigned = getJudgesByCourtrooms(conf_id, courtrooms)
    print('judges:\n%s'%judges)
    # judges is a list of which judges we need to grab data for
    print('Assigned Courtrooms:\n%s'%courtroomsAssigned)
    # courtroom_names is a dict of the courtroom_id with one string (name) and a list of (judges)
    judges = getJudgeInfo(conf_id,judges)
    print('judges with more info?:\n%s'%judges)


def getJudgeInfo(conf_id,judges_dict):
    global sheets_service

    for j in judges_dict.copy():
        #print('\tNeed to find out more info for Judge [%s]'%j)

        RANGE_NAME = '%s!B5:C13'%j
        result = sheets_service.spreadsheets().values().get(
            spreadsheetId=conf_id, range=RANGE_NAME).execute()
        ...
        for lst in nRows:
            if 'Name' in lst[0]:
                info['name'] = str(lst[1])
            if 'prid' in lst[0]:
                info['cm_prid'] = str(lst[1])
            if 'JSON' in lst[0]:
                info['json_url'] = str(lst[1])
            if 'Telephone' in lst[0]:
                info['chambers_tel'] = str(lst[1])
            # others - just have not added them yet

        judges_dict[j]['info'] = info
    return judges_dict
   


def getJudgesByCourtrooms(conf_id, crs_list):
    global sheets_service

    RANGE_NAME = 'Courtrooms & Judges!B5:D10'
    result = sheets_service.spreadsheets().values().get(
        spreadsheetId=conf_id, range=RANGE_NAME).execute()
    nRows = result.get('values') if result.get('values')is not None else 0
    judges = {}
    ...
    for lst in nRows:
        judges_raw = []
        if str(lst[0]) in crs_list:
            courtrooms[str(lst[0])] = {'name': str(lst[1]), 'judges': []}
            jl = lst[2].split(",")
            judges_raw.extend(jl)

        for j in judges_raw:
            if j not in judges: 
                #judges.append(str(j))
                judges[str(j)] = { 'courtroom_id': [], 'info': {} }
                courtrooms[str(lst[0])]['judges'].append(str(j))
            judges[str(j)]['courtroom_id'].append(str(lst[0]))

        #for j in judges_raw:
                #judges[str(j)]['courtroom_id'].append(str(lst[0]))
            

    return judges,courtrooms



def getCourtrooms(conf_id, ds_id):
    global sheets_service

    RANGE_NAME = 'endpoints!D5:F10'
    result = sheets_service.spreadsheets().values().get(
        spreadsheetId=conf_id, range=RANGE_NAME).execute()
    nRows = result.get('values') if result.get('values')is not None else 0
    courtrooms = []
    for lst in nRows:
        if ds_id in str(lst[0]):
            courtrooms = lst[2].split(",")
    courtrooms = [str(r) for r in courtrooms]

    return courtrooms



def getDestinationValues(conf_id, t_id):
    global sheets_service

    RANGE_NAME = '%s!G5:H6'% t_id
    result = sheets_service.spreadsheets().values().get(
        spreadsheetId=conf_id, range=RANGE_NAME).execute()
    nRows = result.get('values') if result.get('values')is not None else 0
    dst = {}
    ...
    RANGE_NAME = '%s!B5:C16'% t_id
    result = sheets_service.spreadsheets().values().get(
        spreadsheetId=conf_id, range=RANGE_NAME).execute()
    nRows = result.get('values') if result.get('values')is not None else 0
    dst['strings'] = {}
    for k,v in nRows:
        dst['strings'][str(k)] = str(v)

    return dst



def getTemplateId(conf_id, ds_id):
    global sheets_service

    RANGE_NAME = 'endpoints!B5:E10'
    result = sheets_service.spreadsheets().values().get(
        spreadsheetId=conf_id, range=RANGE_NAME).execute()
    nRows = result.get('values') if result.get('values')is not None else 0
    t_id = ''
    for ss_city,ss_phyloc,ss_endpoint_id,ss_template_id in nRows:
        if ds_id == str(ss_endpoint_id):
            t_id = str(ss_template_id)

    return t_id


def getTemplateFileId(conf_id, t_id):
    global sheets_service

    RANGE_NAME = 'templates!B5:C10'
    result = sheets_service.spreadsheets().values().get(
        spreadsheetId=conf_id, range=RANGE_NAME).execute()
    nRows = result.get('values') if result.get('values')is not None else 0
    f_id = ''
    ...
    #print('%s | %s | %s | %s'%(str(v1),str(v2),str(v3),str(v4)))
