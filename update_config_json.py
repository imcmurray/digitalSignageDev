...
# Now work on the greeter - main titles

RANGE_NAME = 'greeter!B2:C5'
result = sheets_service.spreadsheets().values().get(
    spreadsheetId=SPREADSHEET_ID, range=RANGE_NAME).execute()
...
for a,b in nRows:
    if a:
        print('ID: %s = %s'%(a,b))
        config['GOOGLE'].setdefault('GREETER',{}).setdefault(a,b)

# Work on greeter template and slate selections
RANGE_NAME = 'greeter!B9:H'
result = sheets_service.spreadsheets().values().get(
    spreadsheetId=SPREADSHEET_ID, range=RANGE_NAME).execute()
...
for row in nRows:
    ...
    for i,v in enumerate(row):
        if i == 0:
            ...
            vals.setdefault(vid,{})
        if i == 1:
            vals[vid]['fileName'] = str(v)
        if i == 2:
            vals[vid]['duration'] = int(v)
        if i == 3:
            vals[vid]['folderId'] = str(v)
        if i == 4:
            vals[vid]['fileId'] = str(v)
        if i == 5:
            vals[vid]['svgFolderId'] = str(v)
        if i == 6:
            vals[vid]['svgFileId'] = str(v)

    config['GOOGLE']['GREETER'].setdefault('slates',[]).append(vals)


    #ind = str(v1.replace(" ","_").lower())
#    print('%s | %s | %s | %s | %s | %s'%(str(v1),str(v2),str(v3),str(v4),str(v5),str(v6)))
#   judge_to_endpoints[judge][ind]={ 'max_hearings': int(v2), 'slide_id': str(v3), 'presentation_id': str(v4) }
#    JUDGE_NAME = '%s!C2:C2' % judge
#    jresult = sheets_service.spreadsheets().values().get(
#        spreadsheetId=SPREADSHEET_ID, range=JUDGE_NAME).execute()
